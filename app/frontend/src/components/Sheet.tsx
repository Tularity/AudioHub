...
export function Sheet({
  ...
  // 传一个内联箭头（`onClose={() => setOpen(false)}`，六处里有两处就是这么写的）
  // 就会让它**每次重渲都重跑一遍**：cleanup 先把焦点还给「触发者」，effect 再把
  // 焦点抢回卡片里的第一个可聚焦元素。而配对面板正开着倒计时，`PairView` 是
  // 1 Hz 重渲的——表现为用户每按一次 Tab，一秒之内就被弹回标题旁的 `?`，
  // 「停止配对」按钮用键盘根本走不到。
  //
  // 修在这里而不是让六个调用方各自 `useCallback`：与「焦点归还」放在组件里而不是
