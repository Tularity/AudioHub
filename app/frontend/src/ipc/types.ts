 ...
 *
 * ⚠ **`PeerState.auto_tier` 不是现状**，别拿它顶替这张表。它是自动降级
 * （plan §16.2）**记在案的判定**：一台钉死 tier 0 的对端，判定可以是 `tier1`
 * （链路确实没有 UDP），而现状照旧是 Tier 0 —— 我们尊重那个钉子。两者通常
 * 一致，分开的那一格恰好是最需要说清楚的一格。
 *
 * 现状至今**没有自己的字段**，唯一真实来源就是这张表：daemon 是按每条连接的
